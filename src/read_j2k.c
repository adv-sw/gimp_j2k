...
Copyright (C) 2002-2020 Advance Software Limited.
...
static void opj_pixel_data_to_gimp(opj_image_t * image, GimpPixelRgn *rgn_in)
{
   int i, j, src_comp;
...
gboolean image_to_gimp(opj_image_t *image, const char *filename, gboolean preview)
{
  ...
  if (preview)
  {
      ...
      preview_layer_ID = gimp_layer_new(preview_image_ID, _("J2K preview"), width, height, layer_type, 100, GIMP_NORMAL_MODE);
  ...
  else
  {
     ...
     layer_ID = gimp_layer_new (image_ID, _("Background"), width, height, layer_type, 100, GIMP_NORMAL_MODE);
  }
  ...
  opj_pixel_data_to_gimp(image, &rgn_in);
